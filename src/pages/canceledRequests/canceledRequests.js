...
const Orders = () => {
  ...
  const getData = ()=>{
    axiosInstance.get("/api/order/allDisActive").then((data)=>{
  ...
  const columns = [
   ...
   
    {
      ...
      {/* <Popconfirm placement="top" title={"Are you sure"} onConfirm={handleConfirm} okText="Yes" cancelText="No">  */}
                    ...
                    {/* </Popconfirm> */}
                    <Popconfirm placement="top" title={"Are you sure"} onConfirm={()=>Activated(record.id)} okText="Yes" cancelText="No"> 
                        <Button type="primary" style={{borderRadius: "7px", marginLeft: "10px"}}>Activete</Button>
                    </Popconfirm>
                    </>
    }
  ];
  ...
  const Activated = (id)=>{
    axiosInstance.patch("/api/order/disCancel/"+id).then((data)=>{
